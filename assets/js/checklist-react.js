...
function ChecklistApp() {
  ...
  const [checklists, setChecklists] = useState(
    loaded?.checklists || window.DEFAULT_DATA?.checklists || []
  );
  const [selectedId, setSelectedId] = useState(
    loaded?.selectedId || checklists[0]?.id || null
  );
  ...
  useEffect(() => {
    saveState({ checklists, selectedId });
  ...
  function resetSelected() {
    if (!selected) return;
    setChecklists((arr) =>
      arr.map((c) => (c.id === selectedId ? { ...c, records: {} } : c))
    );
  }
  function exportJSON() {
    ...
    <div>
          ...
          <div className="hidden md:block sticky top-[64px] max-h-[calc(100vh-80px)] overflow-auto p-3 rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900">
            ...
            <ul className="space-y-1">
              {filtered.map((c) => (
                <li
                  key={c.id}
                  className={`group flex items-center gap-2 p-2 rounded-xl cursor-pointer ${
                    selectedId === c.id
                    ...
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:gap-3">
                      <div className="flex-1">
                        <div className="font-medium">
                          {item.label}{" "}
                          {item.required && (
                            <span className="ml-2 inline-block text-[10px] px-2 py-0.5 rounded-full bg-amber-100 text-amber-800 align-middle">
                              Zorunlu
                            </span>
                          )}
                        </div>
                        <div className="text-xs text-neutral-500">
                          Madde ID: {item.id.slice(0, 8)}…
                        </div>
                      </div>
                      <div className="flex flex-wrap gap-2 items-center">
                        <Select
                          value={selected.records?.[item.id]?.status || ""}
                          onChange={(v) =>
                            upsertRecord(item.id, { status: v })
                          }
                          options={["", "Evet", "Hayır", "Uygulanmaz"]}
                        />
                        <input
                          placeholder="Not"
                          value={selected.records?.[item.id]?.note || ""}
                          onChange={(e) =>
                            upsertRecord(item.id, { note: e.target.value })
                          }
                        ...
                        <input
                          ...
                          value={selected.records?.[item.id]?.photo || ""}
                          onChange={(e) =>
                            upsertRecord(item.id, { photo: e.target.value })
                          }
      ...
      {isAdmin && showEditor && selected && (
        <Modal
          ...
          title={`Maddeleri Düzenle — ${selected.name}`}
        >
          <div className="space-y-3">
              ...
              <div key={it.id} className="flex items-center gap-2">
                ...
                <label className="text-sm flex items-center gap-2">
                  <input
                    ...
                    checked={!!it.required}
                    onChange={(e) =>
                      updateItem(it.id, { required: e.target.checked })
                    }
                  />{" "}
                  Zorunlu
                </label>
...
function Card({ children }) {
  return (
    <div className="p-4 rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 shadow-sm">
      {children}
    </div>
  );
}
